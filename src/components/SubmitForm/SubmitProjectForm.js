...
import { submitProject } from '../../reducers/dashboard/dashboard.actions';

const SubmitProjectForm = ({ isCompleted, isEnrolled, user }) => {
  const dispatch = useDispatch();
  const { id } = useParams();
  const history = useHistory();
  const completedData = useSelector(
    (state) => state.dashboard.project.completed
  );
  const inProgressData = useSelector(
    (state) => state.dashboard.project.inProgress
  );

  const status = useSelector((state) => state.dashboard.project.allId);
  ...
  const submitHandler = async (e) => {
    if (githubLink == null || demoLink == null) {
      ...
      restData = inProgressData.filter(
        (element) => element.projectData.docId[0] !== id
      );
      let tempInProgress = [];
      let tempCompleted = [];
      ...
      dispatch(submitProject({ tempCompleted, tempInProgress, tempAllId }));
      ...
      try {
        await firestore
          .collection(`enroll/${user.id}/projects`)
          .doc(`${id}`)
          .update({
            SubmissionDetails,
            isCompleted: true,
          ...
          <SubmissionRightContent>
            <InstructionsBox>
              <ContentWrapper>
                ...
                <Description>✅Create a public Github Repository</Description>
                <Description>✅Write your project code</Description>
                <Description>✅Get a demo link</Description>
                <Description>✅Submit your solution</Description>
              </ContentWrapper>
            </InstructionsBox>
            <InstructionsYellow>
              <ContentWrapper>
